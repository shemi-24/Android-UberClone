...
public class ViewLocatioMapActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_view_locatio_map);
        ...
        btnRide.setText("I want to give ride to "+getIntent().getStringExtra("pUsername"));
        btnRide.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
//                Toast.makeText(ViewLocatioMapActivity.this, getIntent().getStringExtra("pUsername"), Toast.LENGTH_SHORT).show();
                ParseQuery<ParseObject> carRequestQuery=ParseQuery.getQuery("RequestCar");
                carRequestQuery.whereEqualTo("username",getIntent().getStringExtra("pUsername")).findInBackground(new FindCallback<ParseObject>() {
                    @Override
                    public void done(List<ParseObject> uberRequest, ParseException e) {
                        if(e==null){
                            if(uberRequest.size()>0){
                                for(ParseObject object:uberRequest){
                                    object.put("driverOfMe", ParseUser.getCurrentUser().getUsername());
                                    object.saveInBackground(new SaveCallback() {
                                        @Override
                                        public void done(ParseException e) {
                                           if(e==null){
                                               Intent googleIntent=new Intent(Intent.ACTION_VIEW, Uri.parse("https://maps.google.com/maps?saddr="+getIntent().getDoubleExtra("dLatitude",0)+","+getIntent().getDoubleExtra("dLongitude",0) + "&" + "daddr="+getIntent().getDoubleExtra("pLatitude",0)+","+getIntent().getDoubleExtra("pLongitude",0)));
